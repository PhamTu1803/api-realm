...
class APIService:NSObject {
    ...
    //MARK:- GetEpisode
    func GetEpisode(closure: @escaping (_ response: [EpisodeModel]?, _ error: Error?) -> Void) {
        requestSON("https://rickandmortyapi.com/api/episode", param: nil, method: .GET, loading: true) { (data, error) in
            if let d = data as? [String: Any] {
                var listEpisodeReturn:[EpisodeModel] = [EpisodeModel]()
                if let listEpisode = d["results"] as? [[String : Any]] {
                    for item1 in listEpisode{
                        var EpisodeAdd:EpisodeModel = EpisodeModel()
                        EpisodeAdd = EpisodeAdd.initLoad(item1)
                        listEpisodeReturn.append(EpisodeAdd)
                    }
                }
                closure(listEpisodeReturn, nil)
            }
            else {closure(nil,nil)}
        }
    }
